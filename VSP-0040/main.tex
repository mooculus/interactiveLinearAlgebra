...
By Problem \ref{prob:rowsofRREFlinind}, the nonzero rows of $\mbox{rref}(A)$ are linearly independent.
...
This result was first introduced without proof in \href{https://ximera.osu.edu/linearalgebradzv3/LinearAlgebraInteractiveIntro/SYS-0030/main}{Gaussian Elimination and Rank} where we used it to define the \dfn{rank} of a matrix as the number of nonzero rows in its row-echelon forms.  We can now update the definition of rank as follows.
...
We see that (\ref{eq:init:colspaceB}) has infinitely many solutions.  This means that the columns of $B$ are linearly dependent, and contain a redundant vector.  Which vector can we remove from the set without changing the span?
...
By Problem \ref{prob:pivColsofRREFlinind}, the pivot columns of $\mbox{rref}(B)$ (or $B'$) are linearly independent.  Therefore the corresponding columns of $B$ are linearly independent.  Non-pivot columns can be expressed as linear combinations of the pivot columns, therefore they contribute nothing to the span and can be removed from the spanning set. (See Problem \ref{prob:nonpivColsofRREFlincomb})
\end{proof}
...
To find a basis for $\mbox{null}(A)$ we need to find linearly independent vectors that span $\mbox{null}(A)$.  Take a closer look at the vectors
...
\begin{question} Complete the statement.
