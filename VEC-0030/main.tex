...
\begin{exploration}\label{exp:addInR3}
    ...
    \begin{itemize}
    \item Define vectors $\vec{u}$, $\vec{v}$ and $\vec{w}$.
    \item Use check-boxes at the bottom of the right panel to display the parallelograms. 
    \item RIGHT-CLICK and DRAG the left panel to rotate the graph.
    \end{itemize}
...
\subsection*{Properties of Vector Addition and Scalar Multiplication}
  \begin{theorem}\label{th:vecproperties} The following properties hold for vectors $\vec{u}$, $\vec{v}$ and ${\bf w}$ in $\RR^n$ and scalars $k$ and $p$ in $\RR$.
  \begin{enumerate}
...
\emph{Problems \ref{prob:evaluatevectsumdiff1}-\ref{prob:evaluatevectsumdiff2}}
