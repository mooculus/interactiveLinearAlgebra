...
\draw[thin,gray!40] (-1,-1) grid (4,4);
  ...
  \draw[line width=1pt,-stealth, black](0,0)--(3,2) node[above right]{$\vec{v}=\begin{bmatrix}3\\2\end{bmatrix}$};
 \end{tikzpicture}
\end{center}
...
Use the interactive below to answer the questions.  Tips: To better navigate between the interactive and the questions, we suggest that you use a second device to manipulate the interactive.  Refresh your browser to reset the interactive to default settings.
...
We now return to the question of what makes a good coordinate system.  In this section we restricted our explorations to the plane but the same principles apply to $\RR^n$.  (What does a coordinate grid look like in $\RR^3$?)
