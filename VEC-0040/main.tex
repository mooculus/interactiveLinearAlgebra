...
The last remaining task is to identify the sides of the parallelogram as scalar multiples of $\begin{bmatrix}-1\\3\end{bmatrix}$ and $\begin{bmatrix}-4\\2\end{bmatrix}$.  Observe that vectors $\begin{bmatrix}1\\-3\end{bmatrix}$ and $\begin{bmatrix}2\\-1\end{bmatrix}$ determine the parallelogram.
...
Vector $\begin{bmatrix}2\\-1\end{bmatrix}$ is half the length of $\begin{bmatrix}-4\\2\end{bmatrix}$ and points in the opposite direction, while the vector $\begin{bmatrix}1\\-3\end{bmatrix}$ is the same length as $\begin{bmatrix}-1\\3\end{bmatrix}$ and also points in the opposite direction.
...
\end{problem}
  ...
  \begin{problem}\label{prob:lincombtwovectors4a}
    We have seen Procedure \ref{pro:lincombgeo} applied to vectors in $\RR^2$.  The same process can, in certain cases be applied to vectors in $\RR^3$.  Follow the steps in Procedure \ref{pro:lincombgeo} to express one vector as a linear combination of two given vectors.  

The following GeoGebra interactive shows vectors ${\bf u}=\begin{bmatrix}3\\0\\-1\end{bmatrix}$, ${\bf v}=\begin{bmatrix}1\\-2\\1\end{bmatrix}$ and ${\bf w}=\begin{bmatrix}7\\4\\-5\end{bmatrix}$.  Use the navigation bar to click through the construction steps.
