...
\begin{template}\label{temp:matrixOps}
    ...
    \begin{example}\label{ex:oct_mat_prod_def}
        Implement Definition \ref{def:matrixvectormult} of matrix-vector multiplication in Octave.  Write a routine that computes the product $A\vec{x}$ of matrix $A$ and vector $\vec{x}$ of appropropriate dimensions.

    \begin{explanation}
    To allow the user to enter any appropriately sized combination of $A$ and $\vec{x}$, we will use the \emph{size} command.
    \begin{verbatim}
        % Find the size of matrix A
        [m, n] = size(A);  % m is the number of rows, and n is the number of columns of A
    \end{verbatim}
    We will also have to check that the dimensions of $A$ and $\vec{x}$ are compatable.  To do this we will utilize the \emph{length} command to find the length of $\vec{x}$.
    \begin{verbatim}
        % Find the length of vector x
        l=length(x);
    \end{verbatim}
    If if the number of columns of $A$ doesn't match the number of entries in $\vec{x}$, we will issue a warning.
