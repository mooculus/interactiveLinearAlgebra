...
% For each pair of vectors, $\vec{u}$ and $\vec{v}$, 
%  \begin{enumerate}
%  \item 
%  Find the value of $x$ that makes vectors $\vec{u}$ and $\vec{v}$ perpendicular. 
...
\begin{problem}\label{prob:perpvectors3}
  \begin{enumerate}
    \item Vector $\vec{u}$ parallel to the line $y=mx$ has the form $\vec{u}=k\begin{bmatrix}1\\m\end{bmatrix}$.  Assuming that $m\neq 0$, find the general form for a vector $\vec{v}$ perpendicular to $y=mx$.
      \begin{hint}
        What do you know about the slopes of perpendicular
        lines?
      \end{hint}
    \item Find $\vec{u}\dotp \vec{v}$.
    \item What is true about the dot product of perpendicular vectors?
