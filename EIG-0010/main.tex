...
\end{bmatrix}$.  The following interactive helps us to visualize the matrix transformation associated with $A$.  Given a vector $\vec{x}$ in $\RR^2$ (left panel), its image, $A\vec{x}$, is also in $\RR^2$ (right panel).  Drag the head of the vector $\vec{x}$ to see its image.
...
% }
% https://www.geogebra.org/classic/yr2btbqj
\begin{onlineOnly}
\begin{center}
...
\begin{theorem}\label{th:eigenScalarMult}
    If $\vec{x}$ is an eigenvector of matrix $A$ and $\lambda$ is the corresponding eigenvalue, then every non-zero scalar multiple of $\vec{x}$ is also an eigenvector of $A$ 
...
% https://www.geogebra.org/classic/rspbahj5
...
To follow the computation below, you need to recall that the imaginary unit $i$ is defined by $i^2=-1$.  

% Please refer to \href{https://ximera.osu.edu/linearalgebradzv3/LinearAlgebraInteractiveIntro/APX-0020/main}{Complex Numbers} for more information.
...
\begin{problem}\label{prob:eigenScalarMult}
    Prove that if $\vec{x}$ is an eigenvector of matrix $A$ and $\lambda$ is the corresponding eigenvalue, then every non-zero scalar multiple of $\vec{x}$ is also an eigenvector of $A$ 
 and $\lambda$ is the corresponding eigenvalue. (Theorem \ref{th:eigenScalarMult})
\end{problem}
