...
\begin{problem}\label{prob:Richardson}
Let $A$ be a real square matrix and $b$ be a real vector. Consider \emph{Richardson's Iteration}:
$$x_{k+1} = x_k + \alpha(b-Ax_k)$$
with scalar $\alpha>0.$ Assume the iteration converges. Show that the limit solution $x$ satisfies $Ax= b.$

\begin{hint}
    Rewrite the scheme with iteration matrix $G = I-\alpha A$ and then use the splitting $A = \frac{1}{\alpha} I - \left(\frac{1}{\alpha} I -A\right)$.
\end{hint}
...
The inspiration for Example \ref{ex:rrefFail} came from \href{https://stackoverflow.com/questions/42893111/matlab-rref-function-precision-error-after-12th-column-of-hilbert-matrices}{this} \emph{Stack Overflow} comment. And Problem \ref{prob:Richardson} from \href{https://www-users.cse.umn.edu/~saad/IterMethBook_2ndEd.pdf}{Saad} sec.~4.2 and Example 4.1 p.~116. 
