...
\begin{problem}\label{prob:maxRank} 
Let $A$ be a $m\times n$ real matrix. Show that $\rank A \leq \min(m,n)$.

\begin{hint}
    Use Problem \ref{prob:atmostnlinindinrnproof} from Sec.~5.3.
\end{hint}
\end{problem}

\begin{problem}\label{prob:invariantRank} 
Let $A$ be a $m\times n$ real matrix. Show that $\rank A = \rank(UA) = \rank(AV)$ for invertible real matrices $U$ and $V$.

\begin{hint}
    Use Theorem \ref{th:dimroweqdimcoleqrank} and Problem \ref{prob:rankoftranspose} from Sec.~5.4.
\end{hint}
\end{problem}
...
Ken Kuttler, {\it  A First Course in Linear Algebra}, Lyryx 2017, Open Edition, pp. 227--232. 
 ...
 Problems \ref{prob:maxRank} and \ref{prob:invariantRank} come from Sec.~5.4 of Keith Nicholson's
 \href{https://open.umn.edu/opentextbooks/textbooks/linear-algebra-with-applications}{\it Linear Algebra with Applications}. (CC-BY-NC-SA)
