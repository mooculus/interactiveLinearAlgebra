...
\begin{problem}\label{prob:Penrose}
Let $A$ be a real $m\times n$ matrix. The {\bf pseudoinverse} of $A$ is the unique $n\times m$ matrix $A^+$ such that the following two conditions are satisfied:
\begin{align}
    \tag{{\bf P1}} \label{eq:P1}
    AA^+A = A, \quad A^+AA^+ = A^+, \\
    \tag{{\bf P2}} \label{eq:P2}
    \text{Both $AA^+$ and $A^+A$ are symmetric.}
\end{align}
Show that $A^+ = (A^TA)^{-1}A^T$ for $A$ with linearly independent columns.
\end{problem}

\begin{remark}\label{rk:Penrose}
    Solutions to {\bf P1} are called \emph{middle inverses} and are not unique in general. Penrose's Theorem guarantees uniqueness of middle inverses satisfying {\bf P2}. In case $A$ has linearly independent rows, the pseudoinverse solves the least squares approximation.
\end{remark}

\subsection*{Acknowledgements}
 
Penrose's Theorem and topics in pseudoinverses (cf.~ Problem \ref{prob:Penrose}) can be found in Sec.~8.6.4 in Keith Nicholson's \href{https://open.umn.edu/opentextbooks/textbooks/linear-algebra-with-applications}{\it Linear Algebra with Applications}. (CC-BY-NC-SA)
