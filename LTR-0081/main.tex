...
You should verify that $\tau$ is linear.  (See Practice Problem \ref{prob:taulinear}.)
...
%(In the process, we will also end up proving that $\tau$ is an isomorphism, which is what you were challenged to do in \href{https://ximera.osu.edu/linearalgebradzv3/LinearAlgebraInteractiveIntro/LTR-0060/main}{Isomorphic Vector Spaces}.)
...
Recall that a \dfn{coordinate isomorphism} maps a vector to its coordinate vector with respect to the given ordered basis (Theorem \ref{ex:coordmapiso}).  In the diagram below, let $R$ and $S$ be coordinate isomorphisms with respect to $\mathcal{B}$ and $\mathcal{C}$, respectively.
...
Observe that $T=S\circ\tau\circ R^{-1}$.  Because $T$ is a composition of linear transformations, $T$ itself is linear (Theorem \ref{th:complinear}).  Thus, we should be able to find the standard matrix for $T$.  To do this, find the images of the standard unit vectors and use them to  create the standard matrix $A$ for $T$.
...
% As a side-note, observe that $\tau=S^{-1}\circ T\circ R$.  Observe also that $T$ is invertible because $A$ is invertible.  So, $T$ is an isomorphism.  As a composition of isomorphisms, $\tau$ is an isomorphism (Theorem \ref{th:isocompisiso} of \href{https://ximera.osu.edu/linearalgebradzv3/LinearAlgebraInteractiveIntro/LTR-0060/main}{Isomorphic Vector Spaces}).  While we could have proved this result directly, as you were challenged to do in \href{https://ximera.osu.edu/linearalgebradzv3/LinearAlgebraInteractiveIntro/LTR-0060/main}{Isomorphic Vector Spaces}, this approach is much less tedious.
...
Let's take a look at what this matrix can do for us.  Suppose we want to find the image of $\vec{v}=2\vec{v}_1+\vec{v}_2$ under $T$.  We can compute this directly, as follows: $$T(\vec{v})=T(2\vec{v}_1+\vec{v}_2)=2T(\vec{v}_1)+T(\vec{v}_2)=3\vec{w}_1-2\vec{w}_2$$
...
% In Exploration Problem \ref{init:subtosub} of \href{https://ximera.osu.edu/linearalgebradzv3/LinearAlgebraInteractiveIntro/LTR-0030/main}{Composition and Inverses of Linear Transformations}, we introduced the following set up.
...
% In Example \ref{ex:subtosubinvert} of \href{https://ximera.osu.edu/linearalgebradzv3/LinearAlgebraInteractiveIntro/LTR-0035/main}{Existence of the Inverse of a Linear Transformation}, we proved that $T$ is invertible.  

Conclude that $T$ is invertible by finding the matrix of $T^{-1}$ with respect to ordered bases $\mathcal{C}=\left\{\begin{bmatrix}1\\0\end{bmatrix},\begin{bmatrix}0\\1\end{bmatrix}\right\}$ of $\RR^2$, and $\mathcal{B}=\left\{\begin{bmatrix}1\\0\\0\end{bmatrix}, \begin{bmatrix}1\\1\\1\end{bmatrix}\right\}$ of $V$.
\begin{explanation}
...
Verify that $\tau$ of Exploration \ref{init:taumatrix} is linear.
\end{problem}
