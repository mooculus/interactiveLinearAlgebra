...
\begin{problem}\label{prob_oct_sys_4}
   ...
   \begin{center} 
    \geogebra{xry6un6h}{700}{800}   
    \end{center}
   
\end{problem}

\begin{problem}\label{prob_oct_sys_3}
    Generate a random $20\times 3$ matrix $A$ whose entries are integers $-1$, $0$, and $1$.  Generate a random $20\times 1$ vector $\vec{b}$ whose components are integers $-1$, $0$, and $1$.  (Hint: The following code generates a random $m$ by $n$ matrix with integer entries between (and including) $a$ and $b$:  
    A=randi([a,b],m,n))
